...
const UI = {
  ...
  renderDeckBuilder: () => {
    ...
    // Filter Logic
    if (UI.currentFilter !== "all") keys = keys.filter((k) => CARDS[k].type === UI.currentFilter);
    if (UI.searchQuery) keys = keys.filter((k) => CARDS[k].name.toLowerCase().includes(UI.searchQuery));
    
    // Sort Logic
    keys.sort((a, b) => UI.sortAsc ? CARDS[a].cost - CARDS[b].cost : CARDS[b].cost - CARDS[a].cost);
  ...
  showInfo: (key) => {
    ...
    if (s.radius || s.splashRadius) html += row("Radius", ((s.radius||s.splashRadius)/10).toFixed(1), "⭕");
    ...
    if (d.spawnEffect) html += row("Spawn", d.spawnEffect.type.toUpperCase(), "✨");
  ...
  renderHand: () => {
      ...
      if (nextIcon && GAME.nextCard && CARDS[GAME.nextCard] && nextIcon.innerText !== CARDS[GAME.nextCard].icon) {
      ...
      GAME.playerHand.forEach((k, idx) => {
          ...
          if (!div) {
              ...
              div.onclick = () => { if (!GAME.tiebreaker) { GAME.selectedCardIdx = GAME.selectedCardIdx === idx ? -1 : idx; UI.renderHand(); } };
              con.appendChild(div);
          }
          if (!k || !CARDS[k]) { div.style.visibility = "hidden"; return; }
          div.style.visibility = "visible";
          const d = CARDS[k];
          if (idx === GAME.selectedCardIdx) { if (!div.classList.contains("active")) div.classList.add("active"); } 
          else { if (div.classList.contains("active")) div.classList.remove("active"); }
