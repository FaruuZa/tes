/**
 * PROJECTILE.JS - Fixed Slow Effect & Executioner Weapon Return
 */
...
class Projectile {
  constructor(
    ...
    this.speed =
      projType === "rolling" || projType === "rolling_log"
        ? 4
        : projType === "boomerang"
        ? 6
        : projSpeed;
  ...
  updateBoomerang(game) {
    ...
    if (this.bState === 0) {
      ...
      // --- FIX EXECUTIONER RETURN ---
      if (Utils.getDist(this, this.owner) < 15) {
        this.dead = true;
        this.owner.hasWeapon = true; // KEMBALIKAN SENJATA!
        this.owner.attackTimer = 30; // Delay sebelum lempar lagi
      }
    }
  }
  ...
  applySplashDamage(x, y, game) {
    ...
    if (this.owner && this.owner.key === "healer_spirit") {
      const allies = [...game.units, ...game.buildings];
      allies.forEach((u) => {
        if (
          u.team === this.team &&
          !u.dead &&
          Utils.getDist({ x, y }, u) <= this.splashRadius
        ) {
          u.heal(300);
